<?php 
    ...
    if(isset($_POST['submit'])) {
        ...
        $query1 = mysqli_query($conn,"SELECT * FROM managers");
        ...
        if($flag == 0)
        {
            ...
            if(strlen($filename)!=0) {
                $destFolder = '../manager/uploads/images/staffs/';
                $ext = explode('.',$filename);
                $ext = strtolower(end($ext));
                $newFile = uniqid('', true). '.' . $ext;
                try{
                    if(move_uploaded_file($temp, $destFolder . $newFile)) {
                        $query = "INSERT INTO staffs (name, address, password, email, phone, image, memail, exp, designation, category, completed_projects, projects_count) VALUES (`$name`,`$address`,`$hashp04`,`$email`,`$phone`,`$newFile`,`$memail`,`$experience`,`$designation`,`$category`,0,0)";
                ...
                // echo "not full";
                $query = "INSERT INTO staffs (name, address, password, email, phone, image, memail, exp, designation, category, completed_projects, projects_count) VALUES ('$name','$address','$hashp04','$email','$phone','client.svg','$memail','$experience','$designation','$category',0,0)";
                mysqli_query($conn, $query);
...
<?php
  		if(isset($_SESSION['error'])){
              ...
              setTimeout(()=>{ window.location.href='add_staffs.php'; },2000);
              </script>";
