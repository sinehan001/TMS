								...
								<div class="menu menu-rounded menu-column menu-lg-row my-5 my-lg-0 align-items-stretch fw-semibold px-2 px-lg-0" id="kt_app_header_menu" data-kt-menu="true">
									<div data-kt-menu-trigger="{default: 'click', lg: 'hover'}" data-kt-menu-placement="bottom-start" class="menu-item menu-lg-down-accordion menu-here-bg me-0 me-lg-2">
										<span class="menu-link">
											<span class="menu-title">Admin History</span>
								...
								<div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
									<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
										...
										<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
											...
											<li class="breadcrumb-item text-muted">Task History</li>
										</ul>
									</div>
								</div>
							</div>
							<div id="kt_app_content" class="app-content flex-column-fluid">
									...
									<div class="card mb-5 mb-xl-8">
										<div class="card-header border-0 pt-5">
											<h3 class="card-title align-items-start flex-column">
												<span class="card-label fw-bold fs-3 mb-1">Client History</span>
												...
												<table id="example" class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
													<thead>
														<tr class="fw-bold text-muted">
															<th class="min-w-150px text-start">Client</th>
                                                            <th class="min-w-100px text-start">Staffs</th>
                                                            <th class="min-w-100px text-center">Call Status</th>
                                                            <th class="min-w-100px text-center">Whatsapp Status</th>
                                                            <th class="min-w-100px text-center">Email Status</th>
															<th class="min-w-100px text-center">Assigned Date & Time</th>
															<th class="min-w-100px text-center"> Last Update </th>
														</tr>
													</thead>
													<tbody>
                                                        <?php 
                                                        include './conn.php';
                                                        $sql = "SELECT * FROM admin_table WHERE status != 'completed'";
                                                        $query = mysqli_query($conn, $sql);
														$array = ['<svg xmlns="http://www.w3.org/2000/svg" width="24" aria-labelledby="not-checked" style="fill: red" height="24" viewBox="0 0 24 24"><path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 16.538l-4.592-4.548 4.546-4.587-1.416-1.403-4.545 4.589-4.588-4.543-1.405 1.405 4.593 4.552-4.547 4.592 1.405 1.405 4.555-4.596 4.591 4.55 1.403-1.416z"/></svg><br/><br/><p id="not-checked">Not Checked</p>', 
														'<svg xmlns="http://www.w3.org/2000/svg" width="24" aria-labelledby="checked" style="fill: green" height="24" viewBox="0 0 24 24"><path d="M23.334 11.96c-.713-.726-.872-1.829-.393-2.727.342-.64.366-1.401.064-2.062-.301-.66-.893-1.142-1.601-1.302-.991-.225-1.722-1.067-1.803-2.081-.059-.723-.451-1.378-1.062-1.77-.609-.393-1.367-.478-2.05-.229-.956.347-2.026.032-2.642-.776-.44-.576-1.124-.915-1.85-.915-.725 0-1.409.339-1.849.915-.613.809-1.683 1.124-2.639.777-.682-.248-1.44-.163-2.05.229-.61.392-1.003 1.047-1.061 1.77-.082 1.014-.812 1.857-1.803 2.081-.708.16-1.3.642-1.601 1.302s-.277 1.422.065 2.061c.479.897.32 2.001-.392 2.727-.509.517-.747 1.242-.644 1.96s.536 1.347 1.17 1.7c.888.495 1.352 1.51 1.144 2.505-.147.71.044 1.448.519 1.996.476.549 1.18.844 1.902.798 1.016-.063 1.953.54 2.317 1.489.259.678.82 1.195 1.517 1.399.695.204 1.447.072 2.031-.357.819-.603 1.936-.603 2.754 0 .584.43 1.336.562 2.031.357.697-.204 1.258-.722 1.518-1.399.363-.949 1.301-1.553 2.316-1.489.724.046 1.427-.249 1.902-.798.475-.548.667-1.286.519-1.996-.207-.995.256-2.01 1.145-2.505.633-.354 1.065-.982 1.169-1.7s-.135-1.443-.643-1.96zm-12.584 5.43l-4.5-4.364 1.857-1.857 2.643 2.506 5.643-5.784 1.857 1.857-7.5 7.642z"/></svg><br/><br/><p id="checked">Checked</p>'];
                                                        if(mysqli_num_rows($query) > 0)
                                                        {
                                                            while($row = mysqli_fetch_assoc($query))
                                                            {
																...
																$domain = $split[0];
																$date = date('Y/m/d');
																if($row['call_status'] == 1 && $row['whatsapp_status'] == 1 && $row['email_status'] == 1 && $row['assigned_time'] == $date)
																{
														echo '<tr>
															<td>
																<div class="d-flex align-items-center">
																	<div class="d-flex justify-content-start flex-column">
																		<h5 onclick="window.location.href=\'client_details.php?address='.$domain.'\'" class="client">'.$row["client"].'</h5>
																	</div>
																</div>
															</td>
                                                            <td class="text-start">'.$row["staffs"].'</td>
                                                            <td class="text-center">'.$array[$row["call_status"]].'</td>
                                                            <td class="text-center">'.$array[$row["whatsapp_status"]].'</td>
                                                            <td class="text-center">'.$array[$row["email_status"]].'</td>
															<td class="text-center">'.$row["assigned_time"].'</td>
															<td class="text-center">'.$row["updated_time"].'</td>
														</tr>';
													}
													$date = date('Y/m/d');
													if($row['assigned_time'] < $date)
													{
											echo '<tr>
												<td>
													<div class="d-flex align-items-center">
												...
												<td class="text-center">'.$row["updated_time"].'</td>
                                                    ...
                                                    {
                                                        echo "<td colspan='8'>No records available</td>";
                                                    }
                                                    ?>
													</tbody>
												</table>
								...
								<div id="kt_app_content_container" class="app-container container-xxl">
									<div class="card mb-5 mb-xl-8">
										<div class="card-header border-0 pt-5">
											<h3 class="card-title align-items-start flex-column">
												<span class="card-label fw-bold fs-3 mb-1">File History</span>
												<span class="text-muted mt-1 fw-semibold fs-7">Over 500 members</span>
											</h3>
										</div>
										<div class="card-body py-3">
											<div class="table-responsive">
												<table id="example" class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
													<thead>
														<tr class="fw-bold text-muted">
														    <th class="min-w-50px text-start">ID</th>
															<th class="min-w-150px text-start">filename</th>
															<th class="min-w-100px text-start">File Link</th>
                                                            <th class="min-w-100px text-start">Uploaded Time</th>
														</tr>
													</thead>
													<tbody>
                                                        <?php 
														...
														echo '<tr>
															...
															<td>
																<div class="d-flex align-items-center">
																	<div class="d-flex justify-content-start flex-column">
																		<h5 onclick=window.open("http://localhost/OneHost/marketing/uploads/files/'.$row["filename"].'","_blank") class="client">'.$row["filename"].'</h5>
																	</div>
																</div>
															</td>
                                                            <td class="text-start">'.$row["upload_time"].'</td>
														</tr>';
                                                    } 
                                                    }
                                                    else 
